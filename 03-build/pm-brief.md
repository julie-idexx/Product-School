...
| Visual design system? | **Spot Mantine** ([mantine.spot.idexxi.pet](https://mantine.spot.idexxi.pet)) — the real internal design system, pulled directly from its Storybook: primary Clarity Blue scale, Roboto (body) / Sora (headings), the actual radius (pill buttons, 16px cards) and spacing scale, and its `positive`/`negative` semantic colors. |
