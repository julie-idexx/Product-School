...
Three chained agents in `agents/`, registered together as one system in [`agents/registry.md`](agents/registry.md) — Module 6's autonomous stack, not one-command skills like above but scripts meant to run on a schedule (nightly / Monday 8am / Friday 4pm). All three ran against real data and each caught at least one real bug before being trusted — see each spec's "test run" section for what broke and why.
...
- **`agents/weekly_insight.py`** ([spec](agents/weekly-insight.md)) — Friday 3-2-1 digest (Done / Changed / Watch), reusing `metric_pulse.py`'s math directly and reading `outcome-log.md` for the Anomaly agent's output: an unresolved diagnosis becomes the Watch item, a confirmed one leads Changed. Says "nothing new" honestly rather than relabeling a stale NPS theme as "changed this week."

`outcome-log.md` (project root) accumulates every triggered diagnosis with a `_TBD_` placeholder for "what actually happened" — check it before assuming a past alert was ever resolved. Once entries there get resolved, run the weekly self-review (`agents/learning-loop.md` — trigger: *"Run the weekly learning loop on outcome-log.md."*), which scores each past diagnosis hit/miss/partial and proposes one heuristic update to this file. It proposes; it never edits `CLAUDE.md` on its own.
